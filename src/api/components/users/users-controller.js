...
async function getUsers(request, response, next) {
  try {
    ...
    const sortOrder = (request.query.sort || '').split(':')[1] || 'asc';

    // Filtering
    const filteredUsers = await usersService.getUsersWithFilter(searchTerm);
    ...
    if (sortField) {
      ...
      console.error('Sort field is undefined or null');
    ...
    const usersOnPage = sortedUsers.slice(startIndex, startIndex + pageSize);

    return response.status(200).json(usersOnPage);
  } catch (error) {
    return next(error);
  }
}
